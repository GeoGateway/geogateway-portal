    ...
    if (wmsgf9_select[3] == 1 || $('#color-mapping-checkbox').prop('checked')) {
        ...
        if($('#color-mapping-checkbox').prop('checked')) {
            ...
            if (has_coloring) {
                ...
                highresoverlay = loadWMS(mapA, "http://js-168-89.jetstream-cloud.org/geoserver/InSAR/wms?","InSAR:uid"+uid+"_unw"); 
                // load legend
                var legend_kml = "http://js-168-89.jetstream-cloud.org/uavsarlegend1/uid"+uid+"_unw_default.kmz"; }
                else {
                //load user defined 
                    var imagename = wmsgf9_samples[uid][4] ['image'];
                    var stylesld = wmsgf9_samples[uid][4] ['style'];
                    highresoverlay=loadWMSwithstyle(mapA, "http://js-168-89.jetstream-cloud.org/geoserver/InSAR/wms?","InSAR:"+imagename,stylesld);
                ...
                highresoverlay = loadWMS(mapA, "http://js-168-89.jetstream-cloud.org/geoserver/highres/wms?","highres:uid"+uid+"_unw");
                    // load high-res legend
            var legend_kml = "http://js-168-89.jetstream-cloud.org/highreslegend/2pi.kmz";
            if (parseInt(uid)<=369) {legend_kml = "http://js-168-89.jetstream-cloud.org/highreslegend/pi.kmz";};
            wmsgf9_select_legend_kml =  new google.maps.KmlLayer({
            url: legend_kml,
            preserveViewport:true,
            screenOverlays:true
...
function reset_color_api(){
    ...
    mapA.overlayMapTypes.setAt(0, null);
    //reload
     highresoverlay = loadWMS(mapA, "http://js-168-89.jetstream-cloud.org/geoserver/InSAR/wms?","InSAR:"+imagename);
    // load legend
    var legend_kml = "http://js-168-89.jetstream-cloud.org/uavsarlegend1/"+imagename+"_default.kmz";
        wmsgf9_select_legend_kml =  new google.maps.KmlLayer({
        url: legend_kml,
        preserveViewport:true,
        screenOverlays:true
...
function new_color_api(){
    ...
    mapA.overlayMapTypes.setAt(0, null);
    //reload wms
    highresoverlay=loadWMSwithstyle(mapA, "http://js-168-89.jetstream-cloud.org/geoserver/InSAR/wms?","InSAR:"+datajson['image'],datajson['style']);
